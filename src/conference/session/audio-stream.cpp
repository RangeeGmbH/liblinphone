...
void MS2AudioStream::postConfigureAudioStream(AudioStream *as, LinphoneCore *lc, bool muted){
	...
	// Apply playback and microphone volume
	ms_filter_call_method(as->soundwrite, MS_AUDIO_PLAYBACK_SET_VOLUME_GAIN, &(lc->sound_conf.soft_play_lev));
	ms_filter_call_method(as->soundread, MS_AUDIO_CAPTURE_SET_VOLUME_GAIN, &(lc->sound_conf.soft_mic_lev));
