...
void CallResumeCommand::exec(Daemon* app, const string& args)
{
	...
	if (ist.fail()) {
		...
		current = true;
		if (call == NULL) {
			app->sendResponse(Response("No current call available.", COMMANDNAME_CALL_RESUME, Response::Error));
			return;
		}
	} else {
		call = app->findCall(cid);
		if (call == NULL) {
			app->sendResponse(Response("No call with such id.", COMMANDNAME_CALL_RESUME, Response::Error));
			return;
		}
	}

	if (linphone_call_resume(call) == 0) {
		app->sendResponse(Response(current ? "Current call was resumed" : "Call was resumed", COMMANDNAME_CALL_RESUME, Response::Ok));
	} else {
		app->sendResponse(Response("Error pausing call", COMMANDNAME_CALL_RESUME, Response::Error));
	}
}
