...
void DtmfCommand::exec(Daemon *app, const string& args) {
	...
	ist >> digit_str;
	if (ist.fail()) {
		app->sendResponse(Response("Missing digit parameter.", COMMANDNAME_DTMF, Response::Error));
		return;
	}
	...
	if (isdigit(digit) || (digit == 'A') || (digit == 'B') || (digit == 'C') || (digit == 'D') || (digit == '*') || (digit == '#')) {
		...
		app->sendResponse(Response("Incorrect digit parameter.", COMMANDNAME_DTMF, Response::Error));
	}
}
