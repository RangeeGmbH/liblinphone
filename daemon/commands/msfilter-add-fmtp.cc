...
void MSFilterAddFmtpCommand::exec(Daemon *app, const string& args) {
	...
	ist >> fmtp;
	if (ist.fail()) {
		app->sendResponse(Response("Missing/Incorrect parameter(s).", COMMANDNAME_MSFILTER_ADD_FMTP, Response::Error));
		return;
	}
	if (type.compare("call") == 0) {
		LinphoneCall *call = app->findCall(id);
		if (call == NULL) {
			app->sendResponse(Response("No Call with such id.", COMMANDNAME_MSFILTER_ADD_FMTP, Response::Error));
			return;
		}
		...
		if (astream == NULL || astream->ms.encoder == NULL) {
			app->sendResponse(Response("This call doesn't have an active audio stream.", COMMANDNAME_MSFILTER_ADD_FMTP, Response::Error));
			return;
		}
		...
		if (stream == NULL) {
			app->sendResponse(Response("No Audio Stream with such id.", COMMANDNAME_MSFILTER_ADD_FMTP, Response::Error));
			return;
		}
		ms_filter_call_method(stream->ms.encoder, MS_FILTER_ADD_FMTP, (void *)fmtp.c_str());
	} else {
		app->sendResponse(Response("Incorrect parameter(s).", COMMANDNAME_MSFILTER_ADD_FMTP, Response::Error));
		return;
	}
