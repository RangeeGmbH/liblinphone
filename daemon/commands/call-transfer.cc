...
void CallTransferCommand::exec(Daemon *app, const string &args) {
	...
	if (ist.fail()) {
		app->sendResponse(Response(COMMANDNAME_CALL_TRANSFER, "Invalid command format (wrong call_to_transfer_id parameter)."));
		return;
	}
	if (ist.peek() == EOF) {
		app->sendResponse(
		    Response(COMMANDNAME_CALL_TRANSFER, "Missing call_to_transfer_to_id or sip_uri_to_transfer_to parameter.", Response::Error));
	}
	...
	if (ist.fail()) {
		...
		if (ist.fail()) {
			app->sendResponse(
			    Response(COMMANDNAME_CALL_TRANSFER, "Invalid command format (wrong call_to_transfer_to_id or sip_uri_to_transfer_to parameter."));
			return;
		}
	}
	...
	if (call_to_transfer == NULL) {
		app->sendResponse(Response(COMMANDNAME_CALL_TRANSFER, "No call with such id."));
		return;
	}
	...
	app->sendResponse(Response(COMMANDNAME_CALL_TRANSFER, "Error transferring call."));
}
