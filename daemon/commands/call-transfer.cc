...
void CallTransferCommand::exec(Daemon* app, const string& args)
{
	...
	if (ist.peek() == EOF) {
		app->sendResponse(Response("Missing call_to_transfer_id parameter.", COMMANDNAME_CALL_TRANSFER, Response::Error));
		return;
	}
	...
	if (ist.fail()) {
		app->sendResponse(Response("Invalid command format (wrong call_to_transfer_id parameter).", COMMANDNAME_CALL_TRANSFER, Response::Error));
		return;
	}
	if (ist.peek() == EOF) {
		app->sendResponse(Response("Missing call_to_transfer_to_id or sip_uri_to_transfer_to parameter.", COMMANDNAME_CALL_TRANSFER, Response::Error));
	}
	...
	if (ist.fail()) {
		...
		if (ist.fail()) {
			app->sendResponse(Response("Invalid command format (wrong call_to_transfer_to_id or sip_uri_to_transfer_to parameter.", COMMANDNAME_CALL_TRANSFER, Response::Error));
			return;
		}
	}
	...
	if (call_to_transfer == NULL) {
		app->sendResponse(Response("No call with such id.", COMMANDNAME_CALL_TRANSFER, Response::Error));
		return;
	}
	if (sip_uri_to_transfer_to.empty()) {
		...
		if (linphone_call_transfer_to_another(call_to_transfer, call_to_transfer_to) == 0) {
			...
			app->sendResponse(Response(ostr.str(), COMMANDNAME_CALL_TRANSFER, Response::Ok));
			return;
		}
	} else {
	...
	app->sendResponse(Response("Error transferring call.", COMMANDNAME_CALL_TRANSFER, Response::Error));
}
