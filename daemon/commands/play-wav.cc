...
void PlayWavCommand::exec(Daemon *app, const string& args) {
	...
	if (!lsd) {
		app->sendResponse(Response("The linphone sound daemon (LSD) is not enabled.", COMMANDNAME_PLAY_WAV, Response::Error));
		return;
	}
	...
	if (ist.eof() && (filename.length() == 0)) {
		app->sendResponse(Response("Missing filename parameter.", COMMANDNAME_PLAY_WAV, Response::Error));
		return;
	}
	if (ist.fail()) {
		app->sendResponse(Response("Incorrect filename parameter.", COMMANDNAME_PLAY_WAV, Response::Error));
		return;
	}
