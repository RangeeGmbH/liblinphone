...
void AudioStreamStopCommand::exec(Daemon *app, const string& args) {
	...
	ist >> id;
	if (ist.fail()) {
		app->sendResponse(Response("Missing/Incorrect parameter(s).", COMMANDNAME_AUDIO_STREAM_STOP, Response::Error));
		return;
	}

	AudioStream *stream = app->findAudioStream(id);
	if (stream == NULL) {
		app->sendResponse(Response("No Audio Stream with such id.", COMMANDNAME_AUDIO_STREAM_STOP, Response::Error));
		return;
	}
