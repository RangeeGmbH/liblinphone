...
void MessageCommand::exec(Daemon *app, const std::string &args){
	...
	if (uri.empty()){
		app->sendResponse(Response("Missing uri parameter.", COMMANDNAME_MESSAGE, Response::Error));
		return;
	}
	LinphoneAddress *addr = linphone_factory_create_address(linphone_factory_get(), uri.c_str());
	if (!addr){
		app->sendResponse(Response("Bad sip uri.", COMMANDNAME_MESSAGE, Response::Error));
		return;
	}
	LinphoneChatRoom *cr = linphone_core_get_chat_room(app->getCore(), addr);
	linphone_address_unref(addr);
	if (!cr){
		app->sendResponse(Response("Internal error creating chat room.", COMMANDNAME_MESSAGE, Response::Error));
		return;
	}
	...
	app->sendResponse(Response(ostr.str(), COMMANDNAME_MESSAGE, Response::Ok));
}
